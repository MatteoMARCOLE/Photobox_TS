(() => {
  ...
  // lib/photoloader.ts
  var URL = "https://webetu.iutnc.univ-lorraine.fr/www/canals5/phox/api";
  var Duri = "https://webetu.iutnc.univ-lorraine.fr";
  function loadPicture(idPicture) {
    return fetch(`${URL}/photos/${idPicture}`).then((rep) => {
      if (!rep.ok) {
  ...
  function loadResource(uri) {
    return fetch(`${Duri}${uri}`).then((rep) => {
      if (!rep.ok) {
        return Promise.reject(new Error(rep.statusText));
      }
      return rep.json();
  ...
  function displayPicture(photo) {
    ...
    if (source !== void 0 && source !== null && zone !== null) {
      const template = import_handlebars.default.compile(source.innerHTML);
      const html = template({
        titre: photo.titre,
        descr: photo.descr,
        type: photo.type,
  ...
  function InsertCategory(categorie) {
  ...
  var galerie = null;
  function load() {
    return loadResource("/www/canals5/phox/api/photos/").then((data) => {
      galerie = data;
      return galerie;
    });
  }
  function pageSuivante() {
    if (galerie === null) {
      return Promise.reject(new Error("La galerie n'est pas encore charg\xE9e"));
    }
  ...
  function getPicture(id) {
    loadPicture(id).then((photos) => {
      ...
      getResource(photos).then((categorie) => {
        console.log(categorie);
        InsertCategory(categorie);
      });
      getCommentaires(photos).then((comments) => {
  ...
  function getResource(photo) {
    return loadResource(photo.links.categorie.href);
  }
  function getCommentaires(photo) {
    return loadResource(photo.links.comments.href);
  }
  var bGalerie = document.querySelector("#load_gallery");
  if (bGalerie !== null)
  ...
  var suivante = document.querySelector("#next");
  ...
  var premiere = document.querySelector("#first");
